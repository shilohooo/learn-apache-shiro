 ...
 * @date 2022/1/28 9:40
 */
public class CustomShiroRealm2 implements Realm {
    private final Logger LOG = getLogger(CustomShiroRealm2.class);

    private static final String USERNAME_IN_INI_CONFIG = "wang";
    private static final String PASSWORD_IN_INI_CONFIG = "123";

    /**
     * 获取{@link CustomShiroRealm2}在应用中的唯一名称
     *
     * @return {@link CustomShiroRealm2}在应用中的唯一名称
     * @author shiloh
     * @date 2022/1/28 9:43
     */
    @Override
    ...
    /**
     * 如果该{@link CustomShiroRealm2}支持传入的{@link AuthenticationToken}则返回{@code true}，否则返回{@code false}
     * <p>
     * 返回{@code true}则说明：该{@link CustomShiroRealm2}可以对传入的{@link AuthenticationToken}进行身份验证
     * <p>
     * 这里设置为支持类型为{@link UsernamePasswordToken}的token
     *
     * @return 该 {@link CustomShiroRealm2} 是否支持传入的{@link AuthenticationToken}，
